CREATE PROCEDURE AddProductToWarehouse @IdProduct INT, @IdWarehouse INT, @Amount INT,
                                       @CreatedAt DATETIME,@GeneratedProductWarehouseId INT OUTPUT
AS
BEGIN
...
SET @GeneratedProductWarehouseId = SCOPE_IDENTITY();
COMMIT;
SELECT @GeneratedProductWarehouseId AS GeneratedProductWarehouseId;
END
go
